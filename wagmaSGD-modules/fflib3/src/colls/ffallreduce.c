...
//neighborhood
int ffallreduce_nh(void * sndbuff, void * rcvbuff, int count, int16_t tag, ffoperator_h operator, ffdatatype_h datatype, int options, ffschedule_h * _sched, int step, int groupsize){
    ...
    int mask = 0x1;
    int maxr = (int)ceil((log2(csize)));
    int groupr = (int)ceil((log2(groupsize)));
    mask <<= (step*groupr);
    ...
    //state->tmpbuffs = (ffbuffer_h *) malloc(sizeof(ffbuffer_h)*(maxr));
    state->tmpbuffs = (ffbuffer_h *) malloc(sizeof(ffbuffer_h)*(groupr));
    //for (int i=0; i<maxr; i++){
    for (int i=0; i<groupr; i++){
        FFLOG("Allocating tmp buff %i; count: %i; datatype: %u\n", i, count, datatype);
        ffbuffer_create(NULL, count, datatype, 0, &(state->tmpbuffs[i]));
    }
    //state->tmpbuffs_count = maxr;
    state->tmpbuffs_count = groupr;
    state->datatype = datatype;
    state->count = count;
    ...
    while (rounds < groupr) {
        ...
        if (dst < csize) {
            ...
            //receive data to tmp buffer
            ffrecv_b(state->tmpbuffs[rounds], dst, tag, 0, &recv);            
 
            //accumulate data in tmp buffer and store in recv buffer
            ffcomp_b(state->tmpbuffs[rounds], rb, operator, FFCOMP_DEST_ATOMIC, rb, &comp);    
