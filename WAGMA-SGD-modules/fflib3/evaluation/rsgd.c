...
int main(int argc, char *argv[])
{
    ...
    int rank, size, count, iters, limiter, step, groupsize;
    float slowp;
    if (argc != 6)
    {
        printf("Usage: %s <count> <iters> <limiter> <step> <groupsize>\n", argv[0]);
        exit(1);
    }
    int16_t tag = 0;
    count = atoi(argv[1]);
    ...
    for (int i = 0; i < N; i++)
    {
        ffsolo_allreduce(to_reduce[i], reduced[i], count, tag++, FFSUM, FFFLOAT, 0, limiter, &(solo_allreduce_sched[i]), step, groupsize);
    }
    ...
    for (int i = 0; i < iters; i++)
    {
        ...
        for (int j = 0; j < N; j++)
        {
            ...
            //if(rank==0 || rank==1 || rank==2 || rank==4)
	    //{	
            //    printf("iter=%d, count=%d, result=%.2f, rank=%d \n", i, count, reduced[0][0], rank);
	    //}
