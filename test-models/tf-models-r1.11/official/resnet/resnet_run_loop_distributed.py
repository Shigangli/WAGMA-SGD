...
# change this module to switch to other parallel SGD variants
try:
  #from official.utils.optimizer_sgp_imagenet import DistSGDOptimizer
  from official.utils.optimizer_localsgd_imagenet import DistSGDOptimizer
except: # In case of import errors (ImportError, ModuleNotFoundError)
  DistSGDOptimizer = None
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    if flags.FLAGS.horovod:
      ...
      tf.logging.info('Enabling Deep500 distributed optimizer')
      comm = CommunicationNetwork()
      optimizer = DistSGDOptimizer(optimizer, comm.size)
